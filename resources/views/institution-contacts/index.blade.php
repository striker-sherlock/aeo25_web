<x-admin>
    <div class="container mt-4">
        <div class="card border-0 overflow-hidden shadow rounded-20 mb-5" style="border-radius:20px">
            <div class="card-header bg-secondary"></div>
            <div class="card-body my-3">
                <h1 class="text-primary text-center">Institution Contacts</h1>
                <a class="btn btn-outline-primary rounded-pill  mt-2 mb-3 px-4"
                    href="{{ route('institution-contacts.create') }}">
                    <i class="fa fa-plus" aria-hidden="true"></i> Add New Contact
                </a>

                <div class="table-responsive">
                    <table class="table table-bordered" id="InstitutionContactTable">
                        <thead class="table-info">
                            <tr>
                                <th class="align-middle text-center">PIC Name</th>
                                <th class="align-middle text-center">Institution Type</th>
                                <th class="align-middle text-center">Institution Name</th>
                                <th class="align-middle text-center">Location</th>
                                <th class="align-middle text-center">PIC Institution</th>
                                <th class="align-middle text-center">Email</th>
                                <th class="align-middle text-center">Phone Number</th>
                                <th class="align-middle text-center">IL</th>
                                <th class="align-middle text-center">FL</th>
                                <th class="align-middle text-center">WA</th>
                                <th class="align-middle text-center">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($institutionContacts as $institutionContact)
                                <tr>
                                    <th scope="row">{{ $institutionContact->admin->name }}</th>
                                    <td>{{ $institutionContact->inst_type }} </td>
                                    <td>{{ $institutionContact->institution_name }} </td>
                                    <td>{{ $institutionContact->location }} </td>
                                    <td>{{ $institutionContact->pic_name }} </td>
                                    <td>{{ $institutionContact->email }} </td>
                                    <td>{{ $institutionContact->phone_number }} </td>
                                    <td>{{ $institutionContact->informal_letter_sent }} </td>
                                    <td>{{ $institutionContact->formal_letter_sent }} </td>
                                    <td>{{ $institutionContact->whatsapp_sent }} </td>
    
                                    <td class="d-flex justify-content-center">
                                        <a class="btn btn-primary mx-2"
                                            href="{{ route('institution-contacts.edit', $institutionContact->id) }}"
                                            title="Update Data">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>

                  </div>




            </div>


        </div>

    </div>
    </div>

</x-admin>
