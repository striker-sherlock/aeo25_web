<x-user title="Competition Slot Registration">
    <div class="container mt-5 pb-5">
        ...
        <form action="{{ route('slot-registrations.store') }}" method="POST" enctype="multipart/form-data">
            <div class="row justify-content-start align-items-center g-4">
                ...
                @foreach ($competitions as $competition)
                    @if ($competition->id == 'IA' || $competition->id == 'OBS')
                        @continue
                    @endif
                    <div class="col-md-4">
                                ...
                                <div class="row align-items-center">
                                    ...
                                    <div class="col">
                                        ...
                                        <input type="radio" class="btn-check"
                                            name="quantity[]-{{ $competition->id }}"
                                            id="{{ $competition->id . $i }}" value="{{ $i }}"
                                            {{ $i == 0 ? 'checked' : '' }} hidden
                                            data-field="{{ $competition->name }}"
                                            data-team={{ $competition->need_team == 0 ? 'Person(s)' : 'Team(s)' }}>
                                        <label class="btn btn-outline-primary"
                                            for="{{ $competition->id . $i }}">{{ $i }}</label>
                                    @endfor
                                </div>
                            </div>
                        </div>
        ...
        <div class="alert alert-primary border-0 shadow-sm mb-3" role="alert" style="letter-spacing: .05em">
            <i class="fa-solid fa-triangle-exclamation me-1"></i> <b>Important info!</b> <br>
            N-1 Adjudicator (each institution must register an adjudicator based on how many teams they register to
            the competition with formula N-1, which N is the total of registered teams. For instance, 3 teams must
                            ...
                            <div class="row align-items-center">
                                <div class="col-3">
                                    <img src="/storage/competition_logo/{{ $independentAdju->logo }}"
                                        alt="{{ $independentAdju->name }}'s logo"
                                ...
                                <div class="col">
                                    ...
                                    @for ($i = 0; $i <= $slot; $i++)
                                        <input type="radio" class="btn-check"
                                            name="quantity[]-{{ $independentAdju->id }}"
                                            id="{{ $independentAdju->id . $i }}" value="{{ $i }}"
                                            {{ $i == 0 ? 'checked' : '' }} hidden
                                            data-field="{{ $independentAdju->name }}"
                                            data-team={{ $independentAdju->need_team == 0 ? 'Person(s)' : 'Team(s)' }}>
                                        <label class="btn btn-outline-primary"
                                            for="{{ $independentAdju->id . $i }}">{{ $i }}</label>
                                    @endfor
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-dialog modal-dialog-centered ">
                <div class="modal-content rounded-20 border-0 shadow p-5">
                    ...
                    <div class="">
                        <div class="row">
                            <div class="col">
                                <button type="button" class="close btn rounded-pill btn-outline-secondary w-100"
