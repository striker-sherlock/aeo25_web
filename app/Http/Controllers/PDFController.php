...
class PDFController extends Controller
{
    ...
    public function merchandiseReceipt(MerchandiseTransaction  $transaction)
    {

        $slots = DB::table('merchandise_orders')
            ...
            ->where('merchandise_orders.transaction_id', '=', $transaction->id)
            ->orderby('updated_at', 'DESC')
            ->get();
        ...
        $output = '<!DOCTYPE html>
            ...
            <head>
                <meta charset="utf-8">
                <title>Merchandise Receipt - ' . $transaction->name .  '</title>
                <link rel="preconnect" href="https://fonts.googleapis.com">
                <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
                <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500&display=swap" rel="stylesheet">
                ';
        ...
        $output .= '
                ...
                <body>
                    ...
                    <div id="details" class="clearfix" style="margin-top:20px">
                        ...
                        <h2 class="name">' . $transaction->name . '</h2>
                        <div class="address">' . $transaction->email . '</div>
                        <div class="address">' . $transaction->phone_number . '</div>
                        <div class="email">Receipt ID - ' . $invoiceCode . '</div>';
        $output .= '
                        <div class="email">Time: ' . $latestUpdate . ' (GMT +7)</div>
                        <br>
                        </div>
                    ...
                    <table border="0" cellspacing="0" cellpadding="0">
                        <thead>
                        <tr>
                            <th class="desc" style="text-align:center;">MERCHANDISE</th>
                            <th class="unit">PRICE</th>
                            <th class="qty">QUANTITY</th>
                            <th class="total" colspan="2">TOTAL</th>
                        </tr>
        ...
        ';
        $output  .= '<p>This payment proof has been approved on ' . date("F j, Y H:i", strtotime($transaction->updated_at)) . ' (GMT+7).</p>';
